...
- 各 task は plan.md 本文に `## Task N` セクションで 5 項目テンプレート:
  - **目的** (Why) / **入力** (前提) / **出力** (Deliverable) / **検証** (DoD) / **Notes** (制約・参照)
...
| frontmatter key, `## Task N`, `## DoD verification`, `phase=draft`/`phase=refine`, workflow 識別子 | **英語固定・翻訳禁止** |
...
- ❌ Two-adapters rule 違反: 実装 1 個目で抽象化を導入しない (2 個目が現れて初めて抽象化)
